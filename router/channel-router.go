...
func registerChannelRoutes(apiRouter *gin.RouterGroup) {
	...
	for _, route := range channelPermissionRoutes {
		if isDynamicChannelPath(route.path) {
			continue
		}
		registerChannelPermissionRoute(channelRoute, route)
	}

	for _, route := range channelPermissionRoutes {
		if !isDynamicChannelPath(route.path) || isRootWildcardChannelPath(route.path) {
			continue
		}
		registerChannelPermissionRoute(channelRoute, route)
	}
	...
	for _, route := range channelPermissionRoutes {
		if !isRootWildcardChannelPath(route.path) {
			continue
		}
		registerChannelPermissionRoute(channelRoute, route)
	}
...
func registerChannelPermissionRoute(channelRoute *gin.RouterGroup, route permissionRoute) {
...
func isDynamicChannelPath(path string) bool {
	return strings.Contains(path, "/:")
}

func isRootWildcardChannelPath(path string) bool {
